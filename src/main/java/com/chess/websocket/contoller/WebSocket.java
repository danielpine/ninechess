...
public class WebSocket {
  ...
  public void processRemove(String message, Result result) throws IOException {
    ...
    if (box.getPiece() == 0 || box.getPiece() == this.user.getColor()) {
      result.setType(333);
      result.setMessage("系统：请选择正确的棋子");
      this.sendMessage(result.toString());
    } else {
      // 厂判断 TODO allMill()
      int millCount = NineChess.mill(thisxy, oppo.user.getColor(), room.getMap());
      log.info("mill check get count {}", millCount);
      boolean checkMill = NineChess.checkMill(thisxy, oppo.user.getColor(), room.getMap());
      if (checkMill) {
        result.setType(333);
        result.setMessage("系统：请选择正确的棋子");
        this.sendMessage(result.toString());
  ...
  public void processPlace(String message, Result result) throws IOException {
    ...
    // TODO update map
    int updateMapRes = NineChess.updateMap(thisxy, user.getColor(), room.getMap());
    ...
    int millCount = NineChess.mill(thisxy, user.getColor(), room.getMap());
  ...
  public void processMove(String message, Result result) throws IOException {
    ...
    result.setMessage("系统：" + result.getXy());
    String thisxy = result.getXy();
    boolean canMove = NineChess.canMove(thisxy, user.getColor(), room.getMap());
    ...
    if (canMove) {
      ...
      int millCount = NineChess.mill(thisxy.split(",")[1], user.getColor(), room.getMap());
      // 检查成三
